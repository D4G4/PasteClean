...
describe('sanitizeForGmail', () => {
  ...
  describe('heading to paragraph conversion', () => {
    // Gmail's compose-area sanitizer strips inline font-size from <p>
    // regardless of paste source (verified on-device). Bold survives,
    // size does not — so we don't write a size in the first place.
    it('converts <h1> to <p> with bold (no font-size — Gmail strips it)', () => {
